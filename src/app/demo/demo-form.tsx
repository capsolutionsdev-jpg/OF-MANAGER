...
export function DemoForm() {
  ...
  if (state?.ok) {
    return (
      <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-8 text-center">
        ...
        <h2 className="text-xl font-bold text-[#221F19]">Demande envoyée, merci !</h2>
        ...
        <Link href="/" className="mt-6 inline-flex rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-semibold text-[#221F19] hover:bg-slate-50">
          ← Retour au site
        </Link>
      </div>
    );
  ...
  return (
    <form action={action} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      ...
      <div className="space-y-1.5">
        <label htmlFor="hebergement" className="text-sm font-medium text-[#221F19]">Hébergement souhaité</label>
        <select id="hebergement" name="hebergement" defaultValue="" className="h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-[#2C53C0]">
      ...
      <fieldset className="space-y-2">
        <legend className="text-sm font-medium text-[#221F19]">Formations concernées</legend>
        <div className="flex flex-wrap gap-2">
          {FORMATIONS.map((f) => (
            <label key={f} className="flex cursor-pointer items-center gap-1.5 rounded-full border border-slate-200 px-3 py-1.5 text-sm has-[:checked]:border-[#2C53C0] has-[:checked]:bg-[#2C53C0]/10 has-[:checked]:text-[#2C53C0]">
      ...
      <div className="space-y-1.5">
        <label htmlFor="message" className="text-sm font-medium text-[#221F19]">Votre besoin</label>
        <textarea id="message" name="message" rows={4} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-[#2C53C0] focus:ring-2 focus:ring-[#2C53C0]/20" placeholder="Décrivez votre organisme et vos attentes…" />
      </div>
