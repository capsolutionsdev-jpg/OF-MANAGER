-- ============================================================
-- ACTIVATION RLS — À COLLER UNE FOIS DANS NEON (SQL Editor), branche de PROD.
-- ÉTAPE 1 : remplace le mot de passe sur la ligne "CREATE ROLE" ci-dessous
--           (un seul endroit → entre les quotes, lettres + chiffres uniquement).
-- ÉTAPE 2 : sélectionne TOUT (Ctrl+A) puis clique Run.
-- Inerte tant que l'app se connecte en owner — s'active au basculement Vercel.
-- ============================================================

-- Rôle applicatif non-owner (la RLS s'applique à lui)
CREATE ROLE app_rls LOGIN PASSWORD 'CHANGE_MOI_lettres_et_chiffres' NOBYPASSRLS;

-- Droits de lecture/écriture (sans pouvoir contourner la RLS)
