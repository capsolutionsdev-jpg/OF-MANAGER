...
import { acceptDevis } from "@/lib/actions/devis-actions";

export function DevisAcceptForm({
  token,
  defaultName,
}: {
  token: string;
  ...
  function onSign() {
    ...
    startTransition(async () => {
      const res = await acceptDevis(token, name, dataUrl);
      if (res.ok) {
        toast.success("Devis accepté ! Merci, l'organisme en est informé.");
        router.refresh();
      } else {
  ...
  return (
    <div className="space-y-4 rounded-lg border bg-muted/30 p-4">
      <div className="grid gap-1.5">
        <Label htmlFor="sign-name">Votre nom et qualité</Label>
        <Input
          id="sign-name"
          value={name}
      ...
      <label className="flex items-start gap-2 text-sm">
        ...
        />
        <span>
          Bon pour accord. J&apos;accepte le présent devis et ses conditions. Ma
          signature manuscrite ci-dessus, horodatée, vaut acceptation (adresse IP
          enregistrée).
        </span>
      </label>
      ...
      <Button onClick={onSign} disabled={isPending} className="w-full">
        ...
        {isPending ? "Validation en cours…" : "Accepter et signer le devis"}
      </Button>
    </div>
  );
}
