...
export default function PartenairesPage() {
  return (
    <main className="min-h-screen bg-white text-[#0D1B3E]">
      ...
      <section className="text-center text-white" style={{ background: `radial-gradient(900px 500px at 80% -10%, #3A3550, transparent 60%), linear-gradient(180deg, ${NAVY}, #0D1B3E)` }}>
        <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6">
          ...
          <h1 className="mt-5 text-4xl font-extrabold sm:text-5xl">Nos <span className="text-[#3B6EF5]">partenaires</span> &amp; <span className="text-[#3B6EF5]">clients</span></h1>
      ...
      <section className="mx-auto max-w-5xl px-4 py-16 text-center sm:px-6">
        <p className="text-xs font-bold uppercase tracking-[0.14em] text-[#3B6EF5]">Ils utilisent OFManager</p>
      ...
      <section className="bg-slate-50/70 py-16">
        <div className="mx-auto max-w-5xl px-4 text-center sm:px-6">
          <p className="text-xs font-bold uppercase tracking-[0.14em] text-[#3B6EF5]">Écosystème</p>
          ...
          <div className="mt-10 grid gap-5 text-left sm:grid-cols-3">
            {[["🏛️", "Certificateurs", "INRS, France Compétences, organismes certificateurs des titres & certifications."], ["💶", "Financeurs & OPCO", "Partenaires du financement de la formation professionnelle."], ["🧩", "Partenaires technologiques", "Signature électronique, e-mailing, paiement, intégrations métier."]].map(([ic, t, d]) => (
              <div key={t} className="rounded-2xl border border-slate-200 bg-white p-6">
                <div className="text-2xl">{ic}</div><h3 className="mt-3 font-bold text-[#0D1B3E]">{t}</h3><p className="mt-1.5 text-sm text-slate-600">{d}</p>
              </div>
            ))}
          </div>
