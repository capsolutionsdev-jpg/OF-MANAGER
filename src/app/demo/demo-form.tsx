  ...
  return (
    <form action={action} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
      ...
      <div className="space-y-1.5">
        ...
        <select id="hebergement" name="hebergement" defaultValue="" className="h-11 w-full rounded-lg border border-slate-200 bg-white px-3 text-sm outline-none focus:border-[#1A5FD4]">
      ...
      <fieldset className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {FORMATIONS.map((f) => (
            <label key={f} className="flex cursor-pointer items-center gap-1.5 rounded-full border border-slate-200 px-3 py-1.5 text-sm has-[:checked]:border-[#1A5FD4] has-[:checked]:bg-[#1A5FD4]/10 has-[:checked]:text-[#1A5FD4]">
      ...
      <div className="space-y-1.5">
        ...
        <textarea id="message" name="message" rows={4} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm outline-none focus:border-[#1A5FD4] focus:ring-2 focus:ring-[#1A5FD4]/20" placeholder="Décrivez votre organisme et vos attentes…" />
      </div>
