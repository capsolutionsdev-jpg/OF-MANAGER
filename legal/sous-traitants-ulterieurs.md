...
| **Resend** | Envoi des e-mails transactionnels | Région **UE — Irlande** (`eu-west-1`, via Amazon SES) ; société éditrice établie **aux États-Unis** | DPA + **clauses contractuelles types (CCT)** à signer/archiver | ☐ Signer/archiver le DPA + CCT · ☑ Domaine vérifié + DKIM/SPF en place · ☐ **Créer le DMARC** · ☐ Limiter les données dans les e-mails (pas de données sensibles) |
